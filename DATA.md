...
打卡评价的 `[比对]` 里，红词点一下能播**她自己读的那一段**（`review-play.js`）。
音源不是新落的文件 —— `build.py` 按 `read.json` 里记的源文件名去 `inbox/` 找到原始录音，
在 `dist/` 里**软链**一条过去（不拷贝：源文件几十兆，而 `dist/` 本来就不进 git）。

**这条链路只在本地成立，这是设计不是缺陷**：CI 上 `inbox/` 不存在 → `paths.material()`
返回 None → 页面里一个 `<audio>` 都不出，点红词退回去念那个词（浏览器 TTS，零字节）。
不需要任何开关，也就没有「忘了关」把孩子的声音带上公网的可能 ——
线上那份报告永远是纯文字的。
