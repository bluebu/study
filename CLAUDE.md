...
- **划线红笔黑笔都认**：第一册是橙红马克笔，第二册（`super8/L4`，Heidi）改成
  直接在数字教材上画黑线。判据不共用一套（黑线和正文一个颜色，得看「最长一段
  连续黑」），挑哪支笔按**划中的词数**，OCR 之前先把线擦掉 —— 线画在字上，
  不擦那一行会整行读坏。原委在 `../feeder/CLAUDE.md` 的「划线检测」那节
