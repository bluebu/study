...
storage/
  ...
  spec/               人写的判断和内容
    ...
    methods.txt         学习方法：一课（语文园地也算一课）提炼一个遇到难题时用的办法，
                        名字借那一课（田忌法、庐山法……），配一条方法论 / 心理学常识，
                        **照课本顺序**，三科共用
...
src/
  ...
  assets/
    ...
    methods.css       学习方法（一课一张卡一行：左方法、右常识；底下那张表给大人）
...
- **分类色里刻意没有红** —— 红专属「分数越差越红」。
  唯一的例外是**教材总览**：那张查阅表走老站那套蓝 / 红 / 绿（蓝管结构、
  红管「要背 / 要默写」的格子、绿管语文园地那几行），取值同样在 `palette.css`
  的 `--ov-*`。它**不走语文橙** —— 橙底和语文的三科色是同一支，
  底色和字色撞在一起，这张表唯一的用处（扫出哪几行有活儿）就没了
...
- **多页打印单用 `make fit URL=…` 量**（`tools/fit.mjs`）：它在 print 媒介下量每页
  的内容高度和每一行的自然高度，**逐页报「装得下 / 超出多少」**，另外报「最少
  几页、断在哪一行」供换页点参考。内容高过一页时页脚会被挤到下一张纸上、
  PDF 里多出一张只印页码的白纸，**这在 HTML 源码里看不出来**。
  换页点仍旧是人写在 spec 里的（教材总览是「一单元一页」）——
  一页装几行是行高定的，而表格里一列长文本折几行随内容变，机器猜不出来
...
- 用 `str.replace` 改样式/脚本时**加 assert**，否则锚点写错会静默失败，
  然后你会去调一个根本没改动的文件
- **语文的内容要拿教材核**：`make textbook`（`tools/vs-textbook.py`）——
  抽查单里的词必须在它那几页课文里、教材总览的写字数逐课相加必须等于
  书末写字表的总数。2019 版和 2024 版课次、写字表、日积月累都不一样，
  光看 spec 看不出来（踩过：整整一批抽查单是照 2019 版编的）。
  口径和那批错在哪，见 `src/generator/chinese/CLAUDE.md`
...
- **朗读评价的颗粒度是「一次录音」，不是一页。** 一口气读了两页就是一份数据、
  一份 spec、一份报告，名字写区间（`super8/L3/p68-69`），`--ref` 按朗读顺序
  给几份。跨页**不切文件、不重跑**：每页多少词、错几处、占哪段时间都是从同一份
  对齐里派生的，落进 `read.json` 的 `pages`，报告里出一张 `[分页]` 小表。
  平时看的是目录页上**每天一条的汇总**，单次报告点进去才看。
- **一叠教材截图一次进来，页码机器自己认。** 页角那枚绿圆盘（对开页在外侧，
  左页左下、右页右下；**左页是偶数、右页是奇数**，读数奇偶对不上就当没读出来）
  —— 名字只填到「书/课」（`super8/L3`），17 页一次落成
  ...
  「认出第 63–79 页，中间不缺页」
- **客户端按学科分了工作区**（最左边一条：语文 / 英语 / 数学），英语那边挂着
  **当天的打卡作业** —— 读的就是这边的 `storage/spec/english/homework/<YYYYMMDD>.txt`。
  挂上哪天，录音的名字就从那天的「点读 · 第 N 页 **划线部分**」拼好（`super8/L3/p73-74`），
  不用手敲。所以**打卡单的那条 `* 第 N–M 页 划线部分` 是有下游的**，别改写成别的说法；
  客户端只读不写，群公告怎么整理成 spec 仍旧是会话里的判断
- **超8 是第 8 级，一级 5 本书，一课一个故事**
  （L1《Prince Darling》、L4《Heidi》、L5《格列佛游记》、L6《绿野仙踪》……）。
  ⚠️ **故事名不是书名** —— 一本装几课就是几个故事，拿其中哪一个当书名都是错的
  （这儿原先写着「L4–L6 第二本《Heidi》」，是把第一课的故事名推广成了整本的名字）。
  名字里本来就没有书名这一层：slug 只有「级 / 课 / 页」。
  ⚠️ **每册装几课不固定，别拿课号除以 3** —— 实测第一册 L1–L3、第二册只有
  L4 和 L5、第三册从 L6 起。页码**每本**重新从 1 起，同一本里课与课接着数，
  所以册次是**从页码倒推**的：L5 读到 p70、L6 却从 p7 起，页码掉回个位数就是换了本。
  哪一课在哪一册、实测页码多少，**表在 [DATA.md](DATA.md)**，那儿是唯一真源。
  一课一层目录就够分：`super8/L4/p6`。
  打卡单「点读」抬头的 `Lesson N` 是
  **唯一定得住是第几本的东西**，没写 feeder 就不给名字 —— 见 [DATA.md](DATA.md)。
  ⚠️ **难易在整套书里是分散的，不按册次递进** —— 别拿「换到第二本了」去解释
  分数变化，那是替数字找理由。要说难就给实测的数（口径见
  `src/generator/english/CLAUDE.md` 的「四条不能违反的」第 1 条）
- **划线红笔黑笔都认，一行行划和整块圈起来也都认**：第一册是橙红马克笔，
  第二册（`super8/L4` 起）改成直接在数字教材上画黑线，第三册照旧是黑线。判据不共用一套
  （黑线和正文一个颜色，得看「最长一段连续黑」），挑哪支笔按**划中的词数**，
  OCR 之前先把线擦掉 —— 线画在字上，不擦那一行会整行读坏。
  **笔形还换过一次**：第 5 课起不再一行行划，而是「上下两条长横线 + 一条竖边」
  把一整块圈住，框里的行整块算划中。按下划线那套去认，20 页 2212 词只认出 542 个
  （每个框只捞到下边线上面那一行，还把上边线**外面**那一行错收进来），
  而**光看 `.ref.txt` 的文本看不出来** —— 所以核对图那一步不能省。
  原委和四条配框判据在 `../feeder/CLAUDE.md` 的「划线检测」那节
- **机器认不准的地方它自己会说**：口算卷的手写数字根本不认（题是人抄的）、
  音标认不住就留空、多音字全标出来。这几处别嫌它啰嗦，那正是要人做的判断
- **声学的帧循环不能动**（老站 `analyze.py` 的逐行复刻）。`cd ../feeder && make test`
  拿老站那批数据回归，六个字段加停顿明细必须完全一致 —— 它跑的是 `.legacy` 模式。
  **阈值公式 8/30 换过一次**：从「本底+8、下限−40」改成「说话声级−10」，
  这样七次录音的停顿数才跨录音可比（旧的散在 −21.8 ~ −40，而且撞下限的那几页
  系统性少数了停顿）。历史七份已全部重算回填，**准确率 / WCPM / 分数一个没动** ——
  那几个数是从 spec 的 `errors` 算的，和阈值无关。原委见 `../feeder/CLAUDE.md`
