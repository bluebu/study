...
src/
  ...
  assets/
    ...
    overview.css      教材总览（一份 HTML 两用：纸上打印单 / 手机上横滚速查）
                      正文字号下限 16px；页底空白由 .scroll{flex:1} 吃掉
  ...
  generator/          **所有生成器 .py**，一科一层目录
    chinese/          语文（CLAUDE.md 里有本科的教学准则和三个栏目的口径）
      ...
      overview.py     教材总览：一册一份的总表，背诵 / 默写两列上底色加圆点
                      正文 16px 起（小四），换页点用 make fit 量、写进 spec
...
- **打印单的换页点用 `make fit URL=…` 量**（`tools/fit.mjs`）：它在 print 媒介下量
  各行的自然高度，报最少几页、每页断在哪一行、行高要拉多少。一页装几行是行高
  定的（表格里一列长文本折几行随内容变），**机器猜不出来，所以换页点写在 spec
  里**，这个脚本只负责告诉人写在哪一行后面。它按容量均摊、不贪心装满前几页 ——
  贪心会把零头全留给末页，那就是「空半页」的来路
