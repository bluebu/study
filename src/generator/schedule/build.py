...
两条界限：
  ...
  · **颜色跟着上课的老师走**：科目自己在色表里就用它的色（`MAIN`），
    否则看名字尾巴上的括号（`BY_TEACHER`）—— 「书法（英）」站讲台的是
    英语老师，和英语一个颜色；「劳动（体）」是体育老师来上的，和体育同色
...
# 「劳动（体）」是体育老师来上的，就和体育一个颜色。
BY_TEACHER = {"英": "english", "数": "math", "语": "chinese", "体": "c-drill"}
...
`key` 是色板名：科目自己在色表里就用它的色，否则看括号里的老师本科
