...
- **`.ref.txt` 里冒出不是英文的词，先怀疑扫描认错，别急着写「课本印错」。**
  OCR 在这本书的字体上会把形近字母认岔：`aim → aum`、`Therefore → Theretore`、
  `thunderbolt → thunderboll`。核法是去图上看 —— `p<页>.page.json` 里找那个词的
  `box`，按坐标从 `p<页>.marked.png` 裁出来放大十倍。真是课本印错的也有
  （p74 的 `cat` 该是 `eat`，图上确认过）。两种都不计错，但**报告上的归因不一样**，
  别把扫描的锅扣给课本。
